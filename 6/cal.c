...
drawstr(font, nom, WW(19)/2 - index * 128/(500/secsize) / 2, secsize/3, 500/secsize, 19);
	...
	for (i = 0; i < 18; i++) {
		if (cmonth==(unavail[i]>>5)+format.tm_mon-1&&1-(cmonth<format.tm_mon)) {
		index = unavail[i]&31;
		index = index + 2*((index + w[cmonth])/7) + 9 + w[cmonth];
		RegionFill(index%9*secsize+secsize, index/9*secsize+1, secsize-1, secsize-1, RGB(80, 80, 80), 19); // I prefer round swatches over plain squares
		}
	}

if (cmonth==format.tm_mon) {
index = format.tm_mday;
index = index + 2*((index + w[cmonth])/7) + 9 + w[cmonth];
	x[0] = (index%9+1)*secsize+secsize/4;
	x[1] = x[0]+secsize/4;
	x[2] = x[1];
	x[3] = x[0]+1;
	y[0] = index/9*secsize+secsize/4;
	y[1] = y[0];
	y[2] = y[0]+secsize/4;
	y[3] = y[2];

	drawcurve(RGB(0, 255, 255), x, y, 4, 3, secsize, 19);
	}
...
dreg = getlastused();
if (dreg>>5==format.tm_year) {
// check how many months has gone by
} else {
//clearcache();
}
	...
	while(1) {
		...
		Eve(&c, 19);
			...
			date = index - (2*(index/9) + 1 + w[cmonth]) - 7;
				if (c.x<secsize*8&&c.x>secsize&&1-(cmonth<format.tm_mon)&&1-(date<format.tm_mday)) {
			...
			if (c.t==1) {
				...
				if (c.txt==113) {
				dreg = format.tm_mon + (format.tm_year<<5);
				sign(dreg);
