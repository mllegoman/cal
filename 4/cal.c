...
i = 0;
				// 0 delimits x,y; 1 delimits curve end; 2 delimits chars // 3 delimits font
	while (str[i]) {
	printf("%c\n", str[i]);
	j = 0;
		while (font[j]!=3) {
		...
		printf("%d\n", j);
		if (font[j]==3) {
		RegionFill(x+i*128/dscl, y, 128/dscl, 1, RGB(255, 255, 255), 19);
		RegionFill(x+i*128/dscl, y, 1, 256/dscl, RGB(255, 255, 255), 19);
		RegionFill(x+i*256/dscl, y, 1, 256/dscl, RGB(255, 255, 255), 19);
		...
		o = j;
			...
			pc = 0;
			f = j;
			while (j < o) { // does not know end of file
...
drawstr(font, nom, WW(19)/2 - index * 7 / 2, secsize/2, 20);
